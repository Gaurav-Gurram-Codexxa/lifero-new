...
class PatientController extends AppBaseController
{
    ...
    public function create(Request $request): View
    {
        ...
        return view('patients.create', compact('bloodGroup', 'from'));
    }
    ...
    public function store(CreatePatientRequest $request): RedirectResponse
    {
        ...
        if($request->from == 'b') {
            return redirect()->route('patient-cases.create');
        }
    ...
    public function show(Patient $patient)
    {
        ...
        if (getLoggedInUser()->hasRole('Doctor')) {
            $patientAppointmentHasDoctor = Appointment::whereDoctorId(getLoggedInUser()->owner_id)->where(
                'patient_id',
                $patient->id
            )->exists();
            $birthReportHasDoctor = BirthReport::whereDoctorId(getLoggedInUser()->owner_id)->where(
                'patient_id',
                $patient->id
            )->exists();
            $deathReportHasDoctor = DeathReport::whereDoctorId(getLoggedInUser()->owner_id)->where(
                'patient_id',
                $patient->id
            )->exists();
            $InvestigationReportHasDoctor = InvestigationReport::whereDoctorId(getLoggedInUser()->owner_id)->where(
                'patient_id',
                $patient->id
            )->exists();
            $operationReportHasDoctor = OperationReport::whereDoctorId(getLoggedInUser()->owner_id)->where(
                'patient_id',
                $patient->id
            )->exists();
    ...
    public function destroy(Patient $patient): JsonResponse
    {
        ...
        // $patientModels = [
        //     Appointment::class, PatientCase::class, Prescription::class,
        // ];
    ...
    {
        $term = $request->q;
        $data = Patient::join('users', 'user_id', '=', 'users.id')
            ->select([
