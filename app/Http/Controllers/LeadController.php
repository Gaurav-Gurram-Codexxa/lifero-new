...
class LeadController extends AppBaseController
{
    ...
    public function patientCases(Lead $lead)
    {
        $patient = Patient::join('users', 'users.id', '=', 'patients.user_id')
            ->where('phone', $lead->contact)
            ->select(['patients.id'])
        ...
        return redirect()->route('patient-cases.create', ['p' => $patient->id]);
    }
    ...
    function import(Request $request)
    {
        ...
        Excel::import(new ImportLeads, $file);
        Flash::success('Lead imported successfully');
        return redirect()->route('leads.importLeads');
    }


    public function importLeads()
    {
        return view('leads.importLeads');
    }

    public function convertedLeads()
    {
        return view('leads.convertedLeads');
    }
