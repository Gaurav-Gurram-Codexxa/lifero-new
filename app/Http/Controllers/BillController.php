...
class BillController extends AppBaseController
{
    ...
    public function create(): View
    {
        ...
        $doctors = $this->billRepository->getDoctorList();
        $caseList = $this->billRepository->getCaseList();
        $tax = $this->billRepository->getTaxDetails();
        return view('bills.create',compact('doctors','caseList','tax'))->with($data);
    }
    ...
    public function show(int $id)
    {
        ...
        $taxList = DB::table('bills')
        ->join('tax_models', function ($join) {
            $join->on('bills.tax_id', 'LIKE', DB::raw("CONCAT('%', tax_models.id, '%')"));
        })
        ->select('tax_models.name', 'tax_models.rate')
        ->where('bills.id','=',$id)
        ->get();
        ...
        return view('bills.show',compact('taxList'))->with('bill', $bill);
    }
    ...
    public function edit(Bill $bill)
    {
        ...
        $data['patient'] = Patient::join('users', 'user_id', '=', 'users.id')
            ->select([
    ...
    public function convertToPdf(Bill $bill)
    {
        ...
        $hospital = DB::table('bills')
            ->join('appointments', 'bills.patient_id', '=', 'appointments.patient_id')
            ->join('users', 'appointments.hospital_id', '=', 'users.id')
            ->where('bills.id', '=', $bill->id)
            ->select('users.hospital_name','users.city','users.phone','users.region_code')
            ->first();
    ...
    public function payedit(Bill $bill)
    {
        if (!canAccessRecord(Bill::class, $bill->id)) {
            Flash::error(__('messages.flash.not_allow_access_record'));
        ...
        return view('bills.pay-edit')->with($data);
    }


    public function payBill(Bill $bill, Request $request): JsonResponse
    {
        try {
            DB::beginTransaction();
            $input = $request->all();
            ...
            $bill = $this->billRepository->updateBillAmount($bill->id, $request->all());
    ...
    {
        $caseList = $this->billRepository->getCaseList();
        return $this->sendResponse($caseList, __('messages.flash.retrieve'));
    }
