...
class AppointmentTable extends LivewireTableComponent
{
    ...
    protected $listeners = ['refresh' => '$refresh', 'changeFilter', 'resetPage', 'changeDateFilter','changeFilterSuperAdmin'];
    ...
    public function builder(): Builder
    {
        ...
        ->join('users','appointments.hospital_id','=','users.id')
        ->whereDate('opd_date', Carbon::today())        
        ->where('appointments.hospital_id', '=', Auth::user()->id)
        ->select('appointments.*','users.hospital_name as hname')
        ->with(['patient', 'patient.user', 'doctor', 'doctor.user', 'department']);
        }
    ...
    public function builder1(): Builder
    {
    ...
    // public function builderForSuperAdmin(): Builder
    // {
    //     $query = Appointment::query()
    //     ->join('users','appointments.hospital_id','=','users.id')
    //     ->whereDate('opd_date', Carbon::today())        
    ...
    // public function builderForSuperAdmin1(): Builder
    // {
    //     $query = Appointment::query()
    //     ->join('users','appointments.hospital_id','=','users.id')
    //     ->select('appointments.*','users.hospital_name as hname')        
