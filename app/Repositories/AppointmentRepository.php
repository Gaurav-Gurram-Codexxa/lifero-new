...
class AppointmentRepository extends BaseRepository
{
    ...
    public function getHospitals()
    {
        if(auth()->user()->hasRole('Super Admin'))
        {
            ...
            ->whereRaw('LENGTH(hospital_name) > 0')
        ...
        else if(auth()->user()->hasRole('Tele Caller'))
        {
            $hospitals = DB::table('users')
            ->whereRaw('LENGTH(hospital_name) > 0')
            ->select('hospital_name', 'id')
        ...
        {
            $hospitals = User::where('id',Auth::user()->id)->select('id','hospital_name')
            ->get()
            ->sortBy('hospital_name')
            ->pluck('hospital_name', 'id');
    ...
    public function getDepartments($id)
    {
        ...
        ->join('users','doctor_departments.tenant_id','=','users.tenant_id')
        ->select('doctor_departments.*')
        ->where('users.id','=',$id)
        ->get();
    ...
    public function createNewAppointment($input): bool
    {
        try {
            ...
            $appointmentHospitalId = $input['hospital_id'];
