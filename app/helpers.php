...
function redirectToDashboard(): string
{
    ...
    if ($user->hasRole('Super Admin')) {
        ...
        elseif ($user->hasRole(['Tele Caller'])) {
            return 'leads';
        }
...
function getMenuLinks($menu)
{
    ...
    // MAIN_CASE
    if ($menu == User::MAIN_CASE) {
        $defaultRoute = route('patient-cases.index');
        $subMenus = ['Cases', 'Case Handlers', 'Patient Admissions'];
    }
...
{
   return Schedule::with('doctor')->where('doctor_id', getLoggedInUser()->owner_id)->first('id') ?? 0;
};
