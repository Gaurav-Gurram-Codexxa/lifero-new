...
function loadIssuedItems () {
    ...
    $('#issueItems').select2({
        placeholder: Lang.get('messages.new_change.select_item'),
        width: '100%',
    })
...
listenChange('#itemQuantity', function () {
    ...
    if (quantity <= availableQuantity) {
        ...
        showIssueItemError(Lang.get('messages.issued_item.qty_cannot_be_zero'))
    else
        showIssueItemError(Lang.get('messages.issued_item.qty_must_be_less_than_available_qty'))
})
