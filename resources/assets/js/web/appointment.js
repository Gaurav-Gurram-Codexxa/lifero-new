...
function loadHospitalAppointmentData () {
    ...
    let opdDate = $('#frontAppointmentOPDDate').datepicker({
        ...
        onSelect: function (selectedDate, dateStr) {
            ...
            var weekday = [
                Lang.get('messages.new_change.sunday'),
                Lang.get('messages.new_change.monday'),
                Lang.get('messages.new_change.tuesday'),
                Lang.get('messages.new_change.wednesday'),
                Lang.get('messages.new_change.thursday'),
                Lang.get('messages.new_change.friday'),
                Lang.get('messages.new_change.saturday')]
            ...
            if(typeof doctorId =='undefined'|| doctorId==''||doctorId==null){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#frontAppointmentDoctorId').
                    append($('<option value="">'+Lang.get('messages.web_home.select_doctors')+'</option>'))
    ...
    listenSubmit('#appointmentForm', function (event) {
        ...
        if ($('#frontAppointmentDepartmentId').val() == null || $('#frontAppointmentDepartmentId').val() == '') {
            displayErrorMessage(Lang.get('messages.appointment.please_select_doctor')+' '+Lang.get('messages.department'))
            hideScreenLoader()

            return false
        }

        if ($('#frontAppointmentDoctorId').val() == null || $('#frontAppointmentDoctorId').val() == '') {
            displayErrorMessage(Lang.get('messages.appointment.please_select_doctor'))
            hideScreenLoader()
            return false
        }
        if (selectedTime == null || selectedTime == '') {
            displayErrorMessage(Lang.get('messages.new_change.select_time_slot'))
            hideScreenLoader()
            return false
        }
        // screenLock();
    ...
    function validatePassword () {
        ...
        if (password == '' || confirmPassword == '') {
            displayErrorMessage(Lang.get('messages.new_change.all_required_fields'))
            return false
        }

        if (password !== confirmPassword) {
            displayErrorMessage(Lang.get('messages.new_change.password_not_match'))
            return false
        }
