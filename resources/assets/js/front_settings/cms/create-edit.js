...
function loadFrontSettingCMSData() {
    ...
    if (typeof $('#termConditionPrivacyPolicy').val() != 'undefined' &&
        ...
        let quill2 = new Quill('#privacyPolicyId', {
            ...
            placeholder: Lang.get('messages.web_home.privacy_policy'),
            theme: 'snow', // or 'bubble'
        });
        ...
        listenSubmit('#termsAndCondition', function () {
            ...
            if (quill1.getText().trim().length === 0) {
                displayErrorMessage(Lang.get('messages.new_change.terms_condition_required'));
                return false;
            }

            if (quill2.getText().trim().length === 0) {
                displayErrorMessage(Lang.get('messages.new_change.privacy_policy'));
                return false;
            }
...
function isValidCmsImage(inputSelector, validationMessageSelector) {
    ...
    if ($.inArray(ext, ['jpg', 'png', 'jpeg']) == -1) {
        ...
        $(validationMessageSelector).html(Lang.get('messages.new_change.image_must_be')).show();
        return false;
    }
...
listenSubmit('#addCMSForm', function () {
    ...
    if (isEmpty($.trim(homePageExperience))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_experience')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return false;
    }
    if (isEmpty($.trim(title))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_title')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return false;
    }
    if(isEmpty($.trim(shortDescription))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_description')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageBoxTitle))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_box_title')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageBoxDes))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_box_description')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homeDoctorTextId))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_certified_doctor_text')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homeDoctorTitleId))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_certified_doctor_title')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homeDoctorDescription))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_certified_doctor_description')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageCerBoxTl))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_certified_box_title')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageCerBoxDes))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_certified_box_description')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageStep1Tl))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_step_1_title')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageStep1Des))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_step_1_description')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageStep2Tl))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_step_2_title')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageStep2Des))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_step_2_description')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageStep3Tl))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_step_3_title')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageStep3Des))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_step_3_description')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageStep4Tl))){
        displayErrorMessage(
            Lang.get('messages.front_setting.home_page_step_4_title')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(homePageStep4Tl))){
        displayErrorMessage(
...
listenSubmit('#createAboutUs', function (){
    ...
    if(isEmpty($.trim(aboutUsTitle))){
        displayErrorMessage(
            Lang.get('messages.front_setting.about_us_title')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(aboutUsDes))){
        displayErrorMessage(
            Lang.get('messages.front_setting.about_us_description')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
    if(isEmpty($.trim(aboutUsMission))){
        displayErrorMessage(
            Lang.get('messages.front_setting.about_us_mission')+' '+Lang.get('messages.new_change.field_not_contain_white_space'));
        return  false;
    }
})
