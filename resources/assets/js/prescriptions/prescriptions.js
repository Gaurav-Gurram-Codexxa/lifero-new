            ...
            if (result.success) {
                ...
                if (result.data.status == 1) {
                    $('#showPrescriptionStatus').append(
                        '<span class="badge bg-light-success">'+Lang.get('messages.common.active')+'</span>');
                } else {
                    $('#showPrescriptionStatus').append(
                        '<span class="badge bg-light-danger">'+Lang.get('messages.filter.deactive')+'</span>');
                }
            ...
            if (result.success) {
                if(result.data.available_quantity <= 10){
                    $('#availableQuantity'+uniqueId).text(
                        Lang.get('messages.item.available_quantity')+ ' : ' +
