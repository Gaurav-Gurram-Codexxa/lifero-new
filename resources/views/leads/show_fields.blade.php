            ...
            <div class="col-xxl-5 col-12">
                <div class="d-sm-flex align-items-center mb-5 mb-xxl-0 text-center text-sm-start">
                    <div class="image image-circle image-small">
                        <img src="{{$lead->user->image_url}}" alt="image"/>
                    </div>
                    <div class="ms-0 ms-sm-10 mt-5 mt-sm-0">
                        <span class="badge bg-light-{{ $lead->user->status === 1 ? 'success' : 'danger' }} mb-2">{{ ($lead->user->status === 1) ? __('messages.common.active') : __('messages.common.de_active') }}</span>
                        <h2><a href="#" class="text-decoration-none">{{$lead->user->full_name}}</a></h2>
                        ...
                        <span class="d-flex align-items-start me-sm-5 mb-2 mt-2 text-gray-600 fs-5 justify-content-center justify-content-sm-center">
                            @if(!empty($lead->address->address1) || !empty($lead->address->address2) || !empty($lead->address->city) || !empty($lead->address->zip))
                                <i class="fa-solid fa-location-dot text-gray-600 me-3 mt-1"></i>
                            @endif
                            <span class="text-start"> {{ !empty($lead->address->address1) ? $lead->address->address1 : '' }}{{ !empty($lead->address->address2) ? !empty($lead->address->address1) ? ',' : '' : '' }}
                                {{ empty($lead->address->address1) || !empty($lead->address->address2)  ? !empty($lead->address->address2) ? $lead->address->address2 : '' : '' }}
                                {{!empty($lead->address->city) ? ','.$lead->address->city : ''}} {{ !empty($lead->address->zip) ? ','.$lead->address->zip : '' }}
                            </span>
                        </span>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                ...
                                <div class="col-lg-8">
                                    <span class="fs-5 text-gray-800">{{!empty($lead->user->phone)?$lead->user->region_code.$lead->user->phone:'N/A'}}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                <label class="pb-2 fs-5 text-gray-600">{{ __('messages.user.gender')  }}</label>
                                <div class="col-lg-8">
                                    <span class="fs-5 text-gray-800">{{ $lead->user->gender == 0 ? 'Male' : 'Female' }}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                <label class="pb-2 fs-5 text-gray-600">{{ __('messages.user.blood_group')  }}</label>
                                <div class="col-lg-8">
                                    <span class="fs-6 stext-{{!empty($lead->user->blood_group) ? 'success' : 'danger'}}">{{ !empty($lead->user->blood_group) ? $lead->user->blood_group : __('messages.common.n/a') }}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                <label class="pb-2 fs-5 text-gray-600">{{ __('messages.user.dob')  }}</label>
                                <div class="col-lg-8">
                                    <span class="fs-5 text-gray-800">{{ !empty($lead->user->dob) ? \Carbon\Carbon::parse($lead->user->dob)->translatedFormat('jS M, Y') : __('messages.common.n/a') }}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                ...
                                <div class="col-lg-8">
                                <span
                                        class="fs-5 text-gray-800">{{ !empty($lead->user->designation) ? $lead->user->designation : __('messages.common.n/a') }}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                ...
                                <div class="col-lg-8">
                                <span
                                        class="fs-5 text-gray-800">{{ !empty($lead->user->qualification) ? $lead->user->qualification : __('messages.common.n/a')}}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                ...
                                <div class="col-lg-8">
                                <span
                                        class="fs-5 text-gray-800 me-2">{{ !empty($lead->user->created_at) ? $lead->user->created_at->diffForHumans() : __('messages.common.n/a') }}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                ...
                                <div class="col-lg-8">
                                <span
                                        class="fs-5 text-gray-800 me-2">{{ !empty($lead->user->updated_at) ? $lead->user->updated_at->diffForHumans() : __('messages.common.n/a') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
{{--            <h2>{{__('messages.my_payrolls')}}</h2>--}}
            <livewire:tele_caller-payroll-table tele_callerId="{{$lead->id}}"/>
