...
<div class="com-sm-12">
    ...
    <div class="d-flex justify-content-end">
        <div class="form-group col-sm-3 mb-5">
            ...
            <label for="primaryT" class="form-label d-flex justify-content-between">
                <span>Tax:<span class="required"></span></span>
            </label>

            <select name="primaryTax" class="form-select" id="primaryTax" data-control="select2">
                <option>Select Tax</option>
                @foreach($tax as $t)
                <option value="{{$t->id}}" data-rate="{{ $t->rate }}">{{$t->name}} - {{ $t->rate }}%</option>
                @endforeach
            </select>

            <label for="secondaryT" class="form-label d-flex justify-content-between mt-3">
                <span>Secondary Tax:<span class="required"></span></span>
            </label>

            <select name="secondaryTax" class="form-select" id="secondaryTax" data-control="select2">
                <option>Select Secondary Tax</option>
                @foreach($tax as $t)
                <option value="{{$t->id}}" data-rate="{{ $t->rate }}">{{$t->name}} - {{ $t->rate }}%</option>
                @endforeach
            </select>

            <input type="hidden" id="selectedPrimaryTaxRateInput" name="selectedPrimaryTaxRateInput">
            <input type="hidden" id="selectedSecondaryTaxRateInput" name="selectedSecondaryTaxRateInput">
...
// Case Session Page Starts
  ...
  $('#caseId').on('change', function() {
    ...
    $.ajax({
      ...
      success: function(response)
      {
            ...
            {
                var services = JSON.parse(response.data[index].services);
                var sessionNo = response.data[index].no;
                var rate = 0;

                for (var i = 0; i < services.length; i++)
                {
                    rate += parseFloat(services[i]['rate']);
                }

                $('#selectedSessions').append(`<option value="${sessionNo}" data-id=${rate}>Session No ${sessionNo} - ${rate}</option>`);
            }
  ...
  $('#selectedSessions').on('change', function() {
    const selectedOption = $(this).find('option:selected');
    const selectedSessionsRate = selectedOption.data('id');
    $("#sessionFee").html(selectedSessionsRate);
  });
...
function updateItemNumbers() {
    ...
    $('#addSelectedCase').click(function() {
        ...
        var selectedSessions = $("#selectedSessions").find("option:selected");

        var sele = "All Sessions";

        if(selectedSessions.text().length > 0)
        {
            sele = selectedSessions.text();
        }

        var consultationData = [
            {
                item_name: "Package Name : "+packageName+" (Case ID : "+caseNo+" "+sele+")",
                price: price,
                qty: 1,
                amount: price
            }
...
$('#billSave').click(function(e) {
    ...
    $('#primaryTax, #secondaryTax').select2({
        placeholder: 'Select Tax'
    });

    const initialSecondaryOptions = $('#secondaryTax').html();

    $('#primaryTax').on('change', function() {
        const selectedPrimaryTax = $(this).val();

        const selectedPrimaryRate = parseFloat($(this).find('option:selected').data('rate')) || 0;

        $('#selectedPrimaryTaxRateInput').val(selectedPrimaryRate);


        $('#secondaryTax').html(initialSecondaryOptions);

        $('#secondaryTax option').each(function() {
            if ($(this).val() === selectedPrimaryTax) {
                $(this).remove();
            }
        });

        $('#secondaryTax').select2({
            placeholder: 'Select Secondary Tax'
        });
        updateTotalAmount(selectedPrimaryRate, parseFloat($('#secondaryTax').find('option:selected').data('rate')) || 0);
    });


    $('#secondaryTax').on('change', function() {
        const selectedSecondaryRate = parseFloat($(this).find('option:selected').data('rate')) || 0;
        const selectedPrimaryRate = parseFloat($('#primaryTax').find('option:selected').data('rate')) || 0;

        $('#selectedSecondaryTaxRateInput').val(selectedSecondaryRate);

        updateTotalAmount(selectedPrimaryRate, selectedSecondaryRate);
    });

    function updateTotalAmount(primaryRate, secondaryRate)
    {
        let currentTotalText = $("#totalPrice").text();
        let currentTotal = parseFloat(currentTotalText.replace(/[^0-9.-]+/g,""));

        const subtotal = primaryRate + secondaryRate;
        currentTotal += parseFloat(subtotal);
    }
});
</script>
