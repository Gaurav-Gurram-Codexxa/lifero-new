            ...
            <div class="col-xxl-5 col-12">
                <div class="d-sm-flex align-items-center mb-5 mb-xxl-0 text-center text-sm-start">
                    <div class="image image-circle image-small">
                        <img src="{{$tele_caller->user->image_url}}" alt="image"/>
                    </div>
                    <div class="ms-0 ms-sm-10 mt-5 mt-sm-0">
                        <span class="badge bg-light-{{ $tele_caller->user->status === 1 ? 'success' : 'danger' }} mb-2">{{ ($tele_caller->user->status === 1) ? __('messages.common.active') : __('messages.common.de_active') }}</span>
                        <h2><a href="#" class="text-decoration-none">{{$tele_caller->user->full_name}}</a></h2>
                        <span class="text-gray-600 fs-5">
                            <i class="fa-solid fa-envelope me-1"></i>
                            <a href="mailto: {{$tele_caller->user->email}}"
                               class="text-gray-600 text-decoration-none fs-5">
                                {{$tele_caller->user->email}}
                            </a>
                        </span>
                        <span class="d-flex align-items-start me-sm-5 mb-2 mt-2 text-gray-600 fs-5 justify-content-center justify-content-sm-center">
                            @if(!empty($tele_caller->address->address1) || !empty($tele_caller->address->address2) || !empty($tele_caller->address->city) || !empty($tele_caller->address->zip))
                                <i class="fa-solid fa-location-dot text-gray-600 me-3 mt-1"></i>
                            @endif
                            <span class="text-start"> {{ !empty($tele_caller->address->address1) ? $tele_caller->address->address1 : '' }}{{ !empty($tele_caller->address->address2) ? !empty($tele_caller->address->address1) ? ',' : '' : '' }}
                                {{ empty($tele_caller->address->address1) || !empty($tele_caller->address->address2)  ? !empty($tele_caller->address->address2) ? $tele_caller->address->address2 : '' : '' }}
                                {{!empty($tele_caller->address->city) ? ','.$tele_caller->address->city : ''}} {{ !empty($tele_caller->address->zip) ? ','.$tele_caller->address->zip : '' }}
                            </span>
                        </span>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                ...
                                <div class="col-lg-8">
                                    <span class="fs-5 text-gray-800">{{!empty($tele_caller->user->phone)?$tele_caller->user->region_code.$tele_caller->user->phone:'N/A'}}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                <label class="pb-2 fs-5 text-gray-600">{{ __('messages.user.gender')  }}</label>
                                <div class="col-lg-8">
                                    <span class="fs-5 text-gray-800">{{ $tele_caller->user->gender == 0 ? 'Male' : 'Female' }}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                <label class="pb-2 fs-5 text-gray-600">{{ __('messages.user.blood_group')  }}</label>
                                <div class="col-lg-8">
                                    <span class="fs-6 stext-{{!empty($tele_caller->user->blood_group) ? 'success' : 'danger'}}">{{ !empty($tele_caller->user->blood_group) ? $tele_caller->user->blood_group : __('messages.common.n/a') }}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                <label class="pb-2 fs-5 text-gray-600">{{ __('messages.user.dob')  }}</label>
                                <div class="col-lg-8">
                                    <span class="fs-5 text-gray-800">{{ !empty($tele_caller->user->dob) ? \Carbon\Carbon::parse($tele_caller->user->dob)->translatedFormat('jS M, Y') : __('messages.common.n/a') }}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                ...
                                <div class="col-lg-8">
                                <span
                                        class="fs-5 text-gray-800">{{ !empty($tele_caller->user->designation) ? $tele_caller->user->designation : __('messages.common.n/a') }}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                ...
                                <div class="col-lg-8">
                                <span
                                        class="fs-5 text-gray-800">{{ !empty($tele_caller->user->qualification) ? $tele_caller->user->qualification : __('messages.common.n/a')}}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                ...
                                <div class="col-lg-8">
                                <span
                                        class="fs-5 text-gray-800 me-2">{{ !empty($tele_caller->user->created_at) ? $tele_caller->user->created_at->diffForHumans() : __('messages.common.n/a') }}</span>
                                </div>
                            </div>
                            <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                                ...
                                <div class="col-lg-8">
                                <span
                                        class="fs-5 text-gray-800 me-2">{{ !empty($tele_caller->user->updated_at) ? $tele_caller->user->updated_at->diffForHumans() : __('messages.common.n/a') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
{{--            <h2>{{__('messages.my_payrolls')}}</h2>--}}
            <livewire:tele_caller-payroll-table tele_callerId="{{$tele_caller->id}}"/>
